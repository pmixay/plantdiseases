...
- [ ] Удалить `server/model.py` целиком или переписать импорт
- [ ] Синхронизировать LICENSE и README (Unlicense — проще и честнее)
...
- [ ] Расширить `AnalysisResponse`: добавить поля `regionX`, `regionY`, `regionWidth`, `regionHeight`, `imageWidth`, `imageHeight`
- [ ] Сохранять их в `ScanEntity` (нужна миграция Room v2→v3)
- [ ] `HeatmapOverlayView.setDetectionRegion` должна принимать пиксельные координаты из сервера, нормализовать к размеру ImageView и только тогда рисовать overlay
- [ ] Учесть `scaleType="centerCrop"` — координаты сервера в координатах оригинального фото, ImageView его кропает. Нужна матрица трансформации
- [ ] Для здоровых растений overlay скрывается — это уже работает
- [ ] Добавить toggle «Показать/скрыть область» — жюри сможет сравнить «до» и «после»

### 2.2 Объяснимость: top-3 альтернативных диагноза
- [ ] Прокинуть `all_probs` через `AnalysisResponse` → `ScanEntity` (JSON-строка)
- [ ] На экране результата карточка «Почему мы так решили?»:
...
- [ ] Это главный аргумент для жюри про «explainable AI»

### 2.3 Экран «О модели»
- [ ] В Profile добавить пункт «О модели и точности»
- [ ] Показать: архитектура (MobileNetV3 + EfficientNet), количество параметров (~8M), количество классов, accuracy на валидации (хардкодом из результатов обучения), краткое описание two-stage пайплайна
- [ ] Можно добавить скриншот confusion matrix (из ноутбука) как ассет

### 2.4 Share с изображением
- [ ] Сейчас share только текст. Добавить картинку через `FileProvider` и `Intent.EXTRA_STREAM`
- [ ] Если реализован Grad-CAM — шарить фото с наложенной heatmap (потребует `Bitmap` композиция)
...
- [ ] Обработка 429 Too Many Requests — отдельное сообщение «сервер загружен, подождите»
- [ ] Проверка размера изображения перед отправкой (max 10 MB) — сервер и так отклонит, но лучше не делать лишний запрос
- [ ] Проверка качества фото (Laplacian variance → если < threshold, предупредить «фото размытое»)
- [ ] Состояние загрузки в `ResultActivity` пока `getScanById` возвращает результат (сейчас briefly шот пустой layout)

### 3.2 Room миграции и данные
- [ ] Миграция v2→v3 для новых полей region (после реализации 2.1)
- [ ] Миграция v3→v4 для `all_probs` JSON (после реализации 2.2)
- [ ] Добавить `fallbackToDestructiveMigration()` ТОЛЬКО как крайний вариант — сейчас его нет, и это правильно, но миграции нужно писать аккуратно

### 3.3 Сервер — безопасность и надёжность
- [ ] Валидация разрешения изображения (не принимать 10000x10000 — OOM)
- [ ] Таймаут на инференс (`asyncio.wait_for`) чтобы один «плохой» запрос не блокировал пул
...
### 3.4 Release-сборка
- [ ] Добавить signing config в `build.gradle.kts` (keystore вне VCS, путь в `local.properties`)
- [ ] Собрать подписанный release APK, проверить что ProGuard не ломает Retrofit/Room/Lottie
- [ ] Проверить `minifyEnabled = true` работает корректно
...
- [ ] Pinch-to-zoom на фото растения — сейчас никак не рассмотреть детали heatmap
- [ ] Skeleton loader пока грузится scan из Room
- [ ] Кнопка «Повторить анализ» (re-analyze) — отправить то же фото ещё раз (полезно если было низкое качество)
- [ ] При диагнозе disease — кликабельная ссылка «Подробнее в справочнике»

### 4.4 Общее UI
- [ ] Edge-to-edge display (прозрачный status bar, контент под системными барами)
...
- [ ] Skip онбординга работает, но после первого запуска нет способа его пересмотреть — добавить пункт в Profile «Как пользоваться приложением»
...
|----------|------|-----------|
| Heatmap в одном и том же месте каждый раз | Жюри заметит за 10 секунд | **Блок 2.1** — обязательно |
| Нет метрик точности модели нигде в UI | «А ваш ИИ вообще работает?» | **Блок 2.3** — экран «О модели» |
...
| Нет объяснения «почему именно эта болезнь» | «А что если он ошибся?» | **Блок 2.2** — top-3 + confidence bars |
| Нет режима offline / standalone APK | «А если нет интернета?» | Упомянуть как future work, либо попробовать TFLite конверсию |
| model.py сломан | Если жюри попросит показать код — баг | **Блок 1.3** |
...
| Grad-CAM реальный | Нет (хардкод) | Да | Да + toggle |
| Top-3 диагнозов в UI | Нет | Да | Да + с confidence-bars |
